...
class ParserOffers:
    ...
    def define_location_data(self, block):
        ...
        location_data["lat"] = ""
        location_data["lon"] = ""
        location_data["kindergarten"] = "-1"
        location_data["school"] = "-1"
        location_data["pharmacy"] = "-1"
        location_data["supermarket"] = "-1"
        location_data["polyclinic"] = "-1"
        location_data["dentistry"] = "-1"
        location_data["parking"] = "-1"
        location_data["park"] = "-1"
        location_data["cafe"] = "-1"
        location_data["shopping_centre"] = "-1"
        location_data["fitness_centre"] = "-1"
        location_data["bank"] = "-1"
        '''
        ...
        for index, element in enumerate(elements):
            ...
            address = ""
            for index, geo in enumerate(element.select("a[data-name='GeoLabel']")):
                if index == 0:
                    address = geo.text
                else:
                    address = address + ", " + geo.text
            location_data["address"] = address

            '''
             lon, lat = getLocationByAddress(address).split(" ")
            location_data["lat"] = lat
            location_data["lon"] = lon

            if lon != "-1":
                location_data["kindergarten"] = getPlacesByCoords(lon, lat, "садик")
                location_data["school"] = getPlacesByCoords(lon, lat, "школа")
                location_data["pharmacy"] = getPlacesByCoords(lon, lat, "аптека")
                location_data["supermarket"] = getPlacesByCoords(lon, lat, "магазин")
                location_data["polyclinic"] = getPlacesByCoords(lon, lat, "поликлиника")
                location_data["dentistry"] = getPlacesByCoords(lon, lat, "стоматология")
                location_data["parking"] = getPlacesByCoords(lon, lat, "парковка")
                location_data["park"] = getPlacesByCoords(lon, lat, "парк")
                location_data["cafe"] = getPlacesByCoords(lon, lat, "кафе")
                location_data["shopping_centre"] = getPlacesByCoords(lon, lat, "торговый центр")
                location_data["fitness_centre"] = getPlacesByCoords(lon, lat, "фитнес-клуб")
                location_data["bank"] = getPlacesByCoords(lon, lat, "банк")
            '''
    ...
    def parse_block(self, block):
        ...
        if self.is_latin:
            try:
                ...
                location_data["address"] = transliterate.translit(location_data["address"], reversed=True)
                '''
                location_data["lat"] = transliterate.translit(location_data["lat"], reversed=True)
                location_data["lon"] = transliterate.translit(location_data["lon"], reversed=True)
                location_data["kindergarten"] = transliterate.translit(location_data["kindergarten"], reversed=True)
                location_data["school"] = transliterate.translit(location_data["school"], reversed=True)
                location_data["supermarket"] = transliterate.translit(location_data["supermarket"], reversed=True)
                location_data["bank"] = transliterate.translit(location_data["bank"], reversed=True)
                location_data["parking"] = transliterate.translit(location_data["parking"], reversed=True)
                location_data["pharmacy"] = transliterate.translit(location_data["pharmacy"], reversed=True)
                location_data["park"] = transliterate.translit(location_data["park"], reversed=True)
                location_data["polyclinic"] = transliterate.translit(location_data["polyclinic"], reversed=True)
                location_data["fitness_centre"] = transliterate.translit(location_data["fitness_centre"], reversed=True)
                location_data["shopping_centre"] = transliterate.translit(location_data["shopping_centre"], reversed=True)
                location_data["cafe"] = transliterate.translit(location_data["cafe"], reversed=True)
                location_data["dentistry"] = transliterate.translit(location_data["dentistry"], reversed=True)
                '''
            except:
                pass
...
def getPlacesByCoords(lon, lat, search_type):
    api_key = "315a62d9-69fb-48c6-be4c-3d23f5b392b8"
    radius = 1000
    width = height = radius / 111300
    ...
    if response.status_code == 200:
        ...
        for feature in features:
            ...
            print(f"{name} ({category}): {address} ({latitude}, {longitude})")
            return name
    else:
        print("Ошибка выполнения запроса")
        print(search_type)
